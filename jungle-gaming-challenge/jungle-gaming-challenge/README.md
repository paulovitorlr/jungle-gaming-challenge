...
O `--force-recreate` garante que o script idempotente do LocalStack configure as quatro filas FIFO e suas redrive policies. A API fica em `http://localhost:3000`.
...
Os testes cobrem Money, Wallet, todas as operações, replay histórico, conflito idempotente, rollback transacional, constraints reais, Inbox/redelivery/DLQ, Outbox/leases/restart, referência fora de ordem, 50 duplicatas paralelas, hot wallet, wallets independentes e três instâncias Nest simultâneas. A reconciliação comprova a igualdade final entre saldo e ledger.
