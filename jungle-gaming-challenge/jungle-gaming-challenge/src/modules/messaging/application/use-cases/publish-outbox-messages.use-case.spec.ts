...
function createDependencies(message?: OutboxMessage) {
  const claimDueForPublishing = mock(() =>
    Promise.resolve(message ? [message] : []),
  );
  const updateClaimed = mock(() => Promise.resolve(true));
...
describe('PublishOutboxMessagesUseCase', () => {
  ...
  it('should schedule a retry when the broker rejects the publication', async () => {
    const message = createMessage();
    const { repository, publisher, publish } = createDependencies(message);
  ...
  it('should report a lost lease after the broker accepted the message', async () => {
    const message = createMessage();
    const { repository, publisher, updateClaimed } =
      createDependencies(message);

    updateClaimed.mockResolvedValue(false);
