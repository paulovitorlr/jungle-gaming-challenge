...
describe('OutboxMessage', () => {
  it('should enqueue an integration event', () => {
    ...
    expect(message.aggregateId).toBe('transaction-123');
  ...
  it('should initially be due', () => {
    const message = OutboxMessage.enqueue(TestIntegrationEvent.create());

    expect(message.isDue(new Date('2026-09-02T18:00:00.000Z'))).toBe(true);
  });

  it('should schedule retries with exponential backoff', () => {
    const message = OutboxMessage.enqueue(TestIntegrationEvent.create());

    const firstAttempt = new Date('2026-09-02T18:01:00.000Z');

    message.scheduleRetry(firstAttempt);

    expect(message.attempts).toBe(1);

    expect(message.nextAttemptAt).toEqual(new Date('2026-09-02T18:01:01.000Z'));

    expect(message.isDue(firstAttempt)).toBe(false);

    const secondAttempt = message.nextAttemptAt!;

    message.scheduleRetry(secondAttempt);

    expect(message.attempts).toBe(2);

    expect(message.nextAttemptAt).toEqual(new Date('2026-09-02T18:01:03.000Z'));
  });

  it('should mark the message as published', () => {
    const message = OutboxMessage.enqueue(TestIntegrationEvent.create());

    const publishedAt = new Date('2026-09-02T18:02:00.000Z');
    ...
    expect(message.isDue(new Date('2026-09-02T18:03:00.000Z'))).toBe(false);
  });

  it('should not publish the same message twice', () => {
    const message = OutboxMessage.enqueue(TestIntegrationEvent.create());

    message.markPublished(new Date('2026-09-02T18:02:00.000Z'));

    expect(() =>
      message.markPublished(new Date('2026-09-02T18:03:00.000Z')),
    ).toThrow('Outbox message is already published');
  });
  ...
  it('should not retry a published message', () => {
    ...
    expect(() =>
      message.scheduleRetry(new Date('2026-09-02T18:03:00.000Z')),
    ).toThrow('Published outbox message cannot be retried');
  });

  it('should rehydrate persisted state', () => {
    const nextAttemptAt = new Date('2026-09-02T18:01:01.000Z');
