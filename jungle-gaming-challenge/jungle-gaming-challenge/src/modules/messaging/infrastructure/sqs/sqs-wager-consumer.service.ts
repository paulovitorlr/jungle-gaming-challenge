import {
  Injectable,
  Logger,
  OnModuleDestroy,
  OnModuleInit,
} from '@nestjs/common';

import { ConfigService } from '@nestjs/config';

import {
  DeleteMessageCommand,
  GetQueueUrlCommand,
  type Message,
  ReceiveMessageCommand,
} from '@aws-sdk/client-sqs';

import type { WagerTransactionRequestedMessage } from '../../application/contracts/wager-transaction-requested.message.js';

import { ProcessWagerMessageUseCase } from '../../application/use-cases/process-wager-message.use-case.js';

import { SqsClientService } from './sqs-client.service.js';
import { MetricsService } from '../../../../shared/infrastructure/observability/metrics.service.js';
import { InboxPayloadConflictError } from '../../application/errors/inbox-payload-conflict.error.js';
import { IdempotencyConflictError } from '../../../wagering/application/errors/idempotency-conflict.error.js';
import { InvalidWagerReferenceError } from '../../../wagering/domain/errors/invalid-wager-reference.error.js';
import { InvalidWagerTransactionError } from '../../../wagering/domain/errors/invalid-wager-transaction.error.js';
import { WagerTransactionKind } from '../../../wagering/domain/enums/wager-transaction-kind.enum.js';

class PermanentMessageError extends Error {}

@Injectable()
export class SqsWagerConsumerService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(SqsWagerConsumerService.name);

  private readonly queueName: string;
  private readonly enabled: boolean;

  private stopping = false;
  private consumerLoop?: Promise<void>;

  constructor(
    configService: ConfigService,
    private readonly sqsClient: SqsClientService,
    private readonly processWagerMessage: ProcessWagerMessageUseCase,
    private readonly metrics: MetricsService,
  ) {
    this.queueName = configService.getOrThrow<string>('SQS_WAGER_QUEUE_NAME');
    this.enabled =
      configService.get<string>('SQS_WAGER_CONSUMER_ENABLED') !== 'false';
  }

  onModuleInit(): void {
    if (!this.enabled) return;
    this.consumerLoop = this.consume();
  }

  async onModuleDestroy(): Promise<void> {
    this.stopping = true;

    await this.consumerLoop;
  }

  private async consume(): Promise<void> {
    const queueUrl = await this.resolveQueueUrl();

    this.logger.log(
      JSON.stringify({
        event: 'sqs_consumer_started',
        queueName: this.queueName,
      }),
    );

    while (!this.stopping) {
      try {
        const response = await this.sqsClient.client.send(
          new ReceiveMessageCommand({
            QueueUrl: queueUrl,
            MaxNumberOfMessages: 10,
            WaitTimeSeconds: 20,
            MessageSystemAttributeNames: ['ApproximateReceiveCount'],
          }),
        );

        for (const message of response.Messages ?? []) {
          await this.processMessage(queueUrl, message);
        }
      } catch (error) {
        if (this.stopping) {
          break;
        }

        this.logger.error(
          JSON.stringify({
            event: 'sqs_receive_failed',
            error: this.errorMessage(error),
          }),
        );

        await this.delay(1_000);
      }
    }

    this.logger.log(
      JSON.stringify({
        event: 'sqs_consumer_stopped',
        queueName: this.queueName,
      }),
    );
  }

  private async processMessage(
    queueUrl: string,
    sqsMessage: Message,
  ): Promise<void> {
    const startedAt = Date.now();
    let message: WagerTransactionRequestedMessage | undefined;
    try {
      if (!sqsMessage.Body || !sqsMessage.ReceiptHandle) {
        throw new Error('SQS message is missing body or receipt handle');
      }

      message = this.parseMessage(sqsMessage.Body);

      const result = await this.processWagerMessage.execute(message);

      if (result.wager) {
        this.metrics.increment('wager_transactions_total', {
          status: result.wager.status,
        });
      }
      if (result.duplicateMessage || result.wager?.idempotentReplay) {
        this.metrics.increment('wager_duplicates_total');
      }
      this.metrics.observeLatency(
        'wager_processing_latency_ms',
        Date.now() - startedAt,
      );

      await this.sqsClient.client.send(
        new DeleteMessageCommand({
          QueueUrl: queueUrl,
          ReceiptHandle: sqsMessage.ReceiptHandle,
        }),
      );

      this.logger.log(
        JSON.stringify({
          event: 'sqs_message_processed',
          correlationId: message.data.idempotencyKey,
          messageId: message.messageId,
          transactionId: result.wager?.transactionId,
          duplicate: result.duplicateMessage,
          providerId: message.data.providerId,
          walletId: message.data.walletId,
        }),
      );
    } catch (error) {
      this.metrics.increment('wager_retries_total');
      if (Number(sqsMessage.Attributes?.ApproximateReceiveCount ?? 0) >= 3) {
        this.metrics.increment('wager_dlq_total');
      }
      /*
       * Não deletamos a mensagem.
       * Após o VisibilityTimeout, a SQS a entrega
       * novamente. Depois de três recebimentos,
       * a RedrivePolicy a move para a DLQ.
       */
      this.logger.error(
        JSON.stringify({
          event: 'sqs_message_failed',
          correlationId: message?.data.idempotencyKey,
          messageId: message?.messageId ?? sqsMessage.MessageId,
          transactionId: message?.data.externalTransactionId,
          providerId: message?.data.providerId,
          walletId: message?.data.walletId,
          sqsMessageId: sqsMessage.MessageId,
          receiveCount: sqsMessage.Attributes?.ApproximateReceiveCount,
          category: this.errorCategory(error),
          error: this.errorMessage(error),
        }),
      );
    }
  }

  private async resolveQueueUrl(): Promise<string> {
    const response = await this.sqsClient.client.send(
      new GetQueueUrlCommand({
        QueueName: this.queueName,
      }),
    );

    if (!response.QueueUrl) {
      throw new Error(`SQS queue ${this.queueName} was not found`);
    }

    return response.QueueUrl;
  }

  private parseMessage(body: string): WagerTransactionRequestedMessage {
    const parsed: unknown = JSON.parse(body);

    if (
      !this.isRecord(parsed) ||
      typeof parsed.messageId !== 'string' ||
      parsed.messageId.trim().length === 0 ||
      parsed.type !== 'WagerTransactionRequested' ||
      typeof parsed.occurredAt !== 'string' ||
      !this.isRecord(parsed.data)
    ) {
      throw new PermanentMessageError(
        'Invalid WagerTransactionRequested message',
      );
    }

    const data = parsed.data;
    const required = [
      'providerId',
      'externalTransactionId',
      'idempotencyKey',
      'playerId',
      'walletId',
      'roundId',
      'gameId',
    ] as const;
    const hasRequiredFields = required.every(
      (field) =>
        typeof data[field] === 'string' && data[field].trim().length > 0,
    );
    const hasValidKind = Object.values(WagerTransactionKind).includes(
      data.kind as WagerTransactionKind,
    );
    const hasValidMoney =
      this.isRecord(data.money) &&
      typeof data.money.amount === 'string' &&
      /^\d+(\.\d{1,2})?$/.test(data.money.amount) &&
      typeof data.money.currency === 'string' &&
      /^[A-Z]{3}$/.test(data.money.currency);

    if (!hasRequiredFields || !hasValidKind || !hasValidMoney) {
      throw new PermanentMessageError('Invalid wager transaction data');
    }

    return parsed as unknown as WagerTransactionRequestedMessage;
  }

  private isRecord(value: unknown): value is Record<string, unknown> {
    return value !== null && typeof value === 'object' && !Array.isArray(value);
  }

  private errorMessage(error: unknown): string {
    return error instanceof Error ? error.message : 'Unknown error';
  }

  private errorCategory(error: unknown): 'permanent' | 'transient' {
    return error instanceof SyntaxError ||
      error instanceof PermanentMessageError ||
      error instanceof InboxPayloadConflictError ||
      error instanceof IdempotencyConflictError ||
      error instanceof InvalidWagerReferenceError ||
      error instanceof InvalidWagerTransactionError
      ? 'permanent'
      : 'transient';
  }

  private delay(milliseconds: number): Promise<void> {
    return new Promise((resolve) => {
      setTimeout(resolve, milliseconds);
    });
  }
}
