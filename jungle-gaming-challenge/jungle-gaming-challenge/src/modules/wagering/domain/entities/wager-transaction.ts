...
export class WagerTransaction {
  ...
  static create(props: CreateWagerTransactionProps): WagerTransaction {
    ...
    return new WagerTransaction(
      props.id ?? WagerTransactionId.create(),
      props.providerId,
      props.externalTransactionId,
      props.idempotencyKey,
      props.payloadHash,
  ...
  markPendingReference(resultingBalance?: Money, now: Date = new Date()): void {
    this.ensureNotTerminal();
    ...
    this._resultingBalance = resultingBalance;
    this._nextReferenceAttemptAt = new Date(now.getTime() + 1_000);
  }

  scheduleReferenceRetry(now: Date): void {
    if (this._status !== WagerTransactionStatus.PendingReference) {
      throw new InvalidWagerTransactionError(
        'Only pending reference transactions can be retried',
      );
    }

    this._referenceAttempts += 1;
    const delay = Math.min(1_000 * 2 ** this._referenceAttempts, 300_000);
    this._nextReferenceAttemptAt = new Date(now.getTime() + delay);
  }
