...
const createOutboxRepositoryMock = (): OutboxMessageRepository =>
  ({
    ...
    claimDueForPublishing: mock(),
    updateClaimed: mock().mockResolvedValue(true),
...
describe('ProcessWagerTransactionUseCase', () => {
  it('should process a BET successfully', async () => {
    ...
    const walletRepository = {
      findById: mock().mockResolvedValue(wallet),
    ...
    const wagerTransactionRepository = {
      ...
      findByProviderAndExternalTransactionId: mock(),
    ...
    expect(walletLedgerRepository.add).toHaveBeenCalledTimes(1);

    expect(wagerTransactionRepository.save).toHaveBeenCalledTimes(1);
  ...
  it('should reject the same idempotency key with a different payload', async () => {
    ...
    const wagerTransactionRepository = {
      ...
      findByIdempotencyKey: mock().mockResolvedValue(existingTransaction),
  ...
  it('should retry after a wallet concurrency conflict', async () => {
    ...
    const walletRepository = {
      ...
      update: mock().mockResolvedValueOnce(false),
    };
