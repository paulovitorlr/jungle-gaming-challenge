...
function expectReferenceFailure(
  ...
  try {
    ...
    expect((error as InvalidWagerReferenceError).code).toBe(
      expectedCode,
    );
  }
}
...
it('should prevent transitions from terminal states', () => {
  ...
  expect(() =>
    failed.markProcessed(undefined, new Date()),
  ).toThrow(
...
it('should return credit for opening, win and refund', () => {
  ...
  expect(opening.ledgerDirectionFor()).toBe(
    LedgerDirection.Credit,
  );
  expect(win.ledgerDirectionFor()).toBe(LedgerDirection.Credit);
  expect(refund.ledgerDirectionFor()).toBe(
    LedgerDirection.Credit,
  );
});
