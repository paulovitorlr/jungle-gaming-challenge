...
import { LedgerDirection } from '../../src/modules/wallet/domain/enums/ledger-direction.enum.js';
...
describe('Wager idempotency concurrency', () => {
  ...
  beforeEach(async () => {
    ...
    await em.execute('truncate table "wallet_ledger_entries"');
  ...
  it('should process the same wager concurrently only once', async () => {
    ...
    const input = {
      ...
      externalTransactionId: 'transaction-concurrent-1',

      idempotencyKey: 'provider-a:transaction-concurrent-1',
      ...
      roundId: 'round-concurrent-1',

      gameId: 'game-concurrent-1',
    ...
    const replayResults = results.filter((result) => result.idempotentReplay);
    ...
    const transactions = await verificationEm.find(WagerTransactionOrmEntity, {
      providerId: 'provider-a',
    ...
    const ledgerEntries = await verificationEm.find(WalletLedgerOrmEntity, {
      walletId: walletId.toString(),
    });

    expect(ledgerEntries).toHaveLength(1);

    const outboxMessages = await verificationEm.find(
    ...
    expect(first.balance).toBe('75.00');

    expect(second.balance).toBe('75.00');
  });

  it('should process the same wager 50 times in parallel with only one debit', async () => {
    const walletId = WalletId.create();
    ...
    const results = await Promise.all(
      Array.from({ length: 50 }, () => useCase.execute(input)),
    );
  ...
  it('should process only one of two concurrent bets that compete for the same balance', async () => {
    const walletId = WalletId.create();
